    ...
    public class AdmAccountController : Controller
    {
        ...
        public async Task<IActionResult> ForgotPasswordAsync(ForgotViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (!user)
                {
                    ViewBag.Message = "Tên người dùng hoặc email của bạn sai!";
                    return View(model);
                }
        ...
        {
            ViewBag.Message = "Liên kết đặt lại mật khẩu đã được gửi đến email của bạn. Kiểm tra hộp thư của bạn.";
            return View();
        }
        [HttpGet]
        ...
        public async Task<IActionResult> ChangePassAsync(ChangePasswordViewModel model)
        {
            ...
            ViewBag.Message = "Mật khẩu của bạn sai! Vui lòng lấy lại.";
            return View();
        }
    }
}
